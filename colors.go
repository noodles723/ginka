...
var (
	...
	bbf = color.New(color.Bold, color.FgBlue).SprintfFunc() // bold

	// Render with cyan for info
	cbf = color.New(color.Bold, color.FgCyan).SprintfFunc() // bold
	cf  = color.New(color.FgCyan).SprintfFunc()             // normal
